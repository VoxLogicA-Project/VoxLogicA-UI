...
<div class="flex flex-col h-full">
	...
	<header class="px-4 py-3 border-b border-surface-500/10 flex-none">
		<div class="flex items-center justify-between gap-2">
			...
			>
			<div class="flex-none whitespace-nowrap text-sm">
				<span class="font-bold text-primary-500">{caseViewModel.selectedCases.length}</span>
				<span class="text-surface-600-300-token"
					>/ {caseViewModel.MAX_SELECTED_CASES} cases selected</span
				>
			</div>
		</div>
	</header>
						...
						<div class="mt-2">
							<div class="grid grid-cols-[1fr_auto_1fr_auto] gap-1 items-center">
								<input
									...
									value={filter.label}
									oninput={(e) =>
										runViewModel.updatePrintFilter(i, { label: e.currentTarget.value })}
								/>
								<select
								...
								<input
									...
									value={filter.value}
									oninput={(e) =>
										runViewModel.updatePrintFilter(i, { value: e.currentTarget.value })}
								/>
